                ...
                container.Page(page =>
                {
                    ...
                    page.Header().Height(80).Background("#FFFFFF").Border(1).BorderColor("#E9ECEF").Row(headerRow =>
                    {
                        ...
                        headerRow.RelativeItem().Padding(10).Column(headerContent =>
                        {
                            ...
                            headerContent.Item().Height(5); // Spacing

                            headerContent.Item().Row(middleRow =>
                            {
                                middleRow.RelativeItem().Text($"البيان: {vm.Desc ?? "غير محدد"}")
                                    .FontSize(12).FontColor("#6C757D").AlignRight();
                            });

                            headerContent.Item().Height(5); // Spacing

                            headerContent.Item().Row(bottomRow =>
                            {
                                bottomRow.RelativeItem().Text($"الفئة: {vm.CategoryName ?? "غير محدد"}")
                                    .FontSize(12).FontColor("#6C757D").AlignRight();
                            });
                        });
                    });
